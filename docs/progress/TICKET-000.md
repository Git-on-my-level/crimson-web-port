...
## Notes
- Pages build sets `BASE_PATH` from `actions/configure-pages` output in the workflow.
...
- Added Husky `pre-commit` hook to require `npm run verify` (typecheck + build) before commits; `prepare` now installs hooks (2026-02-04).
- Updated Pages workflow trigger to include `master` in addition to `main` so deployments run from the default branch (2026-02-04).
- Pages build now uses `actions/configure-pages` for base path output instead of hardcoding the repo name (2026-02-04).
